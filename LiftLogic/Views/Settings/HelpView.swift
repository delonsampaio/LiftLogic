    ...
    private var platesAndBarSection: some View {
        Section("Plates & Bar") {
            ...
            FAQRow(
                ...
                answer: "In Settings → Plate Inventory, use the − button next to any enabled plate to set how many of that plate you own. The calculator will never suggest loading more plates than you have available. When you've set quantity limits and a target weight isn't reachable, the readout shows \"Not enough plates in inventory\" — the issue is your inventory, not the math. (If you haven't set any quantity limits, the readout shows the usual \"Closest\" warning instead.) In REV mode, plates at their inventory limit are dimmed and tapping them does nothing, with an amber banner above the rack.",
