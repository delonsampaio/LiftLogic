...
struct HelpView: View {
    ...
    private var proFeaturesSection: some View {
        Section("Pro") {
            ...
            FAQRow(
                ...
                answer: "A kg-only setting that rounds any weight you commit to the nearest increment actually loadable with your enabled plates — so you never end up with a target that isn't physically achievable. Turn it on in Settings. Has no effect in lbs mode.",
                isProFeature: true, userIsPro: isPro
            )
        }
    }
    ...
    private static let allFAQs: [FAQItem] = [
        ...
        .init(section: "Pro", question: "What is Decimal Precision Lock?", answer: "A kg-only setting that rounds any weight you commit to the nearest increment actually loadable with your enabled plates — so you never end up with a target that isn't physically achievable. Turn it on in Settings. Has no effect in lbs mode.", isPro: true),
    ]
}
